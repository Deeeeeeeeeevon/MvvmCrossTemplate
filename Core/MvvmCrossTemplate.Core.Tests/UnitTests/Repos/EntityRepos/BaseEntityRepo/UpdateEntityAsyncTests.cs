...
{
    [TestFixture]
    public class UpdateEntityAsyncTests : BaseUnitTest
    {
        [Test]
        public async Task WHEN_DatabaseService_Insert_fails_SHOULD_return_Fail_Result()
        {
            ...
            var sut = new BaseEntityRepoBuilder()
                .Where_DatabaseService_UpdateAsync_returns(insertResult)
                .Create();

            //Act
            var result = await sut.UpdateEntityAsync(new BaseEntityBuilder().Create());
        ...
        [Test]
        public async Task WHEN_DatabaseService_Insert_succeeds_SHOULD_return_Success_Result()
        {
            //Arrange
            Result<BaseEntity> insertResult = new ResultOfTypeBuilder<BaseEntity>()
                .With_IsSuccess(true).With_Value(new BaseEntityBuilder().With_Id(123).Create())
                .Create();
            var sut = new BaseEntityRepoBuilder()
                .Where_DatabaseService_UpdateAsync_returns(insertResult)
                .Create();
            ...
            //Act
            Result<BaseEntity> result = await sut.UpdateEntityAsync(new BaseEntityBuilder().Create());

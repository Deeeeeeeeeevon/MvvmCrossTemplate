...
namespace MvvmCrossTemplate.Core.Tests.UnitTests.Repos.EntityRepos.BaseEntityRepo
{
    ...
    [TestFixture]
    public class DeleteAllEntitiesAsyncTests : BaseUnitTest
    {
        [Test]
        public async Task WHEN_DatabaseService_fails_SHOULD_return_Fail_Result()
        {
            ...
            var sut = new BaseEntityRepoBuilder()
                .Where_databaseservice_DeleteAllAsync_returns<BaseEntity>(databaseServiceResult)
                .Create();

            //Act
            Result result = await sut.DeleteAllEntitiesAsync();

            //Assert
            Assert.That(result.IsFailure);
            Assert.That(result.Error.ErrorStack.Count, Is.EqualTo(2));
            Assert.That(result.Error.SourceError.ClassName, Is.EqualTo(typeof(BaseEntityRepo<BaseEntity>).Name));
        }
